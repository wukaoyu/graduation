...
import { Input, Select, Checkbox, Button, Upload, Radio, message } from 'antd'
...
class SingleElectionEditor extends React.Component {
              ...
              <Radio.Group value={questionData.answerTrue[0]}>
                {
                  questionData.answerJson.map((item, index) => {
                    return (
                      <Radio key={item.key} value={index} className='queEditor-list-content-radio' onChange={e => this.changeAnswertrue(e)}>
  ...
  addAnswerOption = () => {
    ...
    newArray.push({answer: '', key: newArray[newArray.length - 1].key + 1})
    this.setState({
      questionData: {
